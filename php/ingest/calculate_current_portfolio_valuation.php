...
if (php_sapi_name() !== 'cli') {
    fwrite(STDERR, "calculate_current_portfolio_valuation: CLI only\n");
    exit(1);
}

require_once '/var/www/html/php/crypto-con.php';

const CALCULATION_VERSION = 'current_portfolio_v1';
...
$accountType = 'SPOT';
$priceReferenceTime = (int) floor(microtime(true) * 1000);

function usage($message = null)
{
    if ($message !== null) {
        fwrite(STDERR, "calculate_current_portfolio_valuation: {$message}\n");
    }
...
function valid_account_type($value)
{
    return is_string($value) && preg_match('/^[A-Z0-9_]{1,32}$/', $value);
}

function parse_args(array $argv, &$accountType)
{
...
function load_latest_balance_snapshot_time(mysqli $db, $accountType)
{
    ...
    return $row ? (string) $row['snapshot_time'] : null;
}

function load_nonzero_holdings(mysqli $db, $accountType, $snapshotTime)
{
    $stmt = $db->prepare(
        "SELECT asset, total
         FROM crypto.balance_snapshots
...
function load_latest_candles(mysqli $db, array $symbols, $priceReferenceTime)
{
    if (count($symbols) === 0) {
        return [];
    }
    ...
    $params[] = $priceReferenceTime;
    foreach (INTERVAL_ORDER as $intervalName) {
        $params[] = $intervalName;
    }

    $sql = "SELECT mk.symbol, mk.interval_name, mk.close_time, mk.close_price
            ...
            ORDER BY mk.symbol ASC,
                     FIELD(mk.interval_name, '1m', '3m', '5m', '15m', '30m', '1h', '2h', '4h', '6h', '8h', '12h', '1d') ASC,
                     mk.close_time DESC";
...
function calculate_current_valuation(mysqli $db, $accountType, $priceReferenceTime)
{
    $balanceSnapshotTime = load_latest_balance_snapshot_time($db, $accountType);
    if ($balanceSnapshotTime === null) {
        throw new RuntimeException('No balance snapshot found for account type ' . $accountType . '.');
    }

    $holdings = load_nonzero_holdings($db, $accountType, $balanceSnapshotTime);
    $totalNonzeroAssetCount = count($holdings);

    $expectedPairs = [];
    foreach ($holdings as $holding) {
        if (!is_parity_asset($holding['asset'])) {
            $expectedPairs[] = $holding['asset'] . 'USDT';
        }
    }
    $expectedPairs = array_values(array_unique($expectedPairs));
    sort($expectedPairs, SORT_STRING);

    $supportedSymbols = load_supported_symbols($db, $expectedPairs);
    $latestCandles = load_latest_candles($db, array_keys($supportedSymbols), $priceReferenceTime);
    ...
    foreach ($holdings as $holding) {
        ...
        $closeTime = (int) $candle['close_time'];
        $closePrice = decimal_from_value($candle['close_price']);
        $ageMs = max(0, (int) $priceReferenceTime - $closeTime);
    ...
    return [
        ...
        'balance_snapshot_time' => $balanceSnapshotTime,
        'price_reference_time' => (string) $priceReferenceTime,
...
function upsert_current_valuation(mysqli $db, array $payload)
{
    $stmt = $db->prepare(
        "INSERT INTO crypto.current_portfolio_valuations
            (account_type, balance_snapshot_time, price_reference_time, estimated_value_usdt,
             stablecoin_value_usdt, valued_asset_count, total_nonzero_asset_count,
             missing_asset_count, coverage_percentage, fresh_price_count, aging_price_count,
             stale_price_count, missing_price_count, missing_assets, price_sources,
             calculation_version)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE
            balance_snapshot_time = VALUES(balance_snapshot_time),
            price_reference_time = VALUES(price_reference_time),
    ...
    $stmt->bind_param(
        ...
        $payload['balance_snapshot_time'],
        $payload['price_reference_time'],
...
try {
    parse_args($argv, $accountType);

    if (!valid_account_type($accountType)) {
    ...
    $tableCheck = $db->prepare(
        "SELECT 1
         ...
         WHERE table_schema = DATABASE()
           AND table_name = 'current_portfolio_valuations'
         LIMIT 1"
    );
    $tableCheck->execute();
    ...
    if (!$tableExists) {
        fail('current_portfolio_valuations table does not exist.');
    }

    $payload = calculate_current_valuation($db, $accountType, $priceReferenceTime);

    if (!$db->begin_transaction()) {
        fail('Failed to start database transaction.');
    }

    upsert_current_valuation($db, $payload);
    $db->commit();

    $snapshotLine = sprintf(
        'calculate_current_portfolio_valuation: account_type=%s balance_snapshot_time=%s price_reference_time=%s total_assets=%d valued_assets=%d missing_assets=%d coverage=%s estimated_value=%s status=completed',
        $payload['account_type'],
        $payload['balance_snapshot_time'],
        $payload['price_reference_time'],
        $payload['total_nonzero_asset_count'],
    ...
    fwrite(STDOUT, $snapshotLine . "\n");

    fwrite(
        STDOUT,
        sprintf(
            'calculate_current_portfolio_valuation: snapshots_attempted=1 snapshots_completed=1 snapshots_failed=0 status=completed'
        ) . "\n"
    );
    exit(0);
} catch (Throwable $e) {
    ...
    fwrite(
        STDOUT,
        sprintf(
            'calculate_current_portfolio_valuation: account_type=%s status=failed error=%s',
            $accountType,
            preg_replace('/[\\r\\n]+/', ' ', $e->getMessage())
        ) . "\n"
    );
    fwrite(STDOUT, "calculate_current_portfolio_valuation: snapshots_attempted=1 snapshots_completed=0 snapshots_failed=1 status=failed\n");
    exit(1);
}
